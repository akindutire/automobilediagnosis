...
<title>Auto repair| Diagnose</title>
...
<div id="templatemo_content_wrapper">
	<div id="templatemo_content">
    	...
    	<div id="column_w530">
            ...
            <div class="header_02">Diagnose <?php $clid=$_GET['clt']; $sql=mysqli_query($link,"SELECT cardiagnosed FROM clients WHERE id='$clid'");	list($car)=mysqli_fetch_row($sql); echo $car; ?></div>
            
            
            
            
            <center><div id="form_cont" style="alignment-adjust:central;">
...
-->
        ...
        <form action="ifaults.php" method="post">
			 <div class="all"><label></label><input type="hidden" value="<?php echo $clid; ?>" name="clt" id="clt"/></div>

            <div class="all"><label>Issue, Undesired Event</label><input type="text" name="undesiredevent" id="undesiredevent"/></div>
            
          
            
        	
            <div class="all"><label>Faulty Scope</label><select name="scope" id="scope">
            
            <?php	
				$isql1=mysqli_query($link,"SELECT * FROM faultscope");
				while(list($id,$scope)=mysqli_fetch_row($isql1)){
					echo "<option value='$id'>$scope</option>";
					}
			
			 ?>
            
            </select></div>
             <div class="all"><label>Sub System</label><cc><select name="sbsys" id="sbsys">
        ...
        <div class="all"><label></label><button type="submit">Diagnose</button></div>
        </form>
